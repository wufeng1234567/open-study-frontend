    ...
    <div v-loading="loading" class="notice-list">
      <div v-for="notice in noticeList" :key="notice.noticeId || notice.id" class="notice-card"
        ...
        <div class="notice-body">
          ...
          <div class="notice-summary">{{ notice.noticeContent || notice.content }}</div>
      ...
      <el-empty v-if="!loading && noticeList.length === 0" description="暂无 @ 消息" />
    </div>
  </div>
</template>
...
const fetchMentions = async () => {
  ...
  try {
    ...
    const all = res.data || []
    const mentions = all.filter(n => {
      if (!n.remark || n.remark === '') return false
      try {
        const remark = JSON.parse(n.remark)
        return remark.type === 'mention'
      } catch (e) {
        return false
      }
    })
    noticeList.value = mentions
    unreadCount.value = mentions.length
  } catch (e) {
    ElMessage.error('获取 @ 消息失败')
  } finally {
    loading.value = false
  }
}
...
const handleClick = (notice) => {
  ...
  if (id) {
    ...
    refreshBadge('mention', unreadCount.value)
    markRead(id).catch(() => { })
    console.log('=== mentions markRead 完成: noticeId=' + id)
    console.log('=== mentions 调用 refreshBadge: type=mention, count=' + unreadCount.value)
  }
...
const handleReadAll = () => {
  ...
  if (!ids) return
  unreadCount.value = 0
  refreshBadge('mention', 0)
  ElMessage.success('已全部标记为已读')
  markReadAll(ids).catch(() => { })
  console.log('=== mentions markReadAll 完成: ids=' + ids)
  console.log('=== mentions 调用 refreshBadge: type=mention, count=0')
}

const handleDelete = async (notice) => {
  const id = notice.noticeId || notice.id
  try {
    await ElMessageBox.confirm('确定要删除这条 @ 通知吗？', '提示', {
...
const handleDeleteAll = async () => {
  ...
  try {
    await ElMessageBox.confirm('确定要清空全部 @ 通知吗？', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    })
